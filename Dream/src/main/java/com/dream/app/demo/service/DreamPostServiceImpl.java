...
import com.dream.app.demo.repository.CommentRepository;
import com.dream.app.demo.repository.DreamPostRepository;
import com.dream.app.demo.repository.KeywordRepository;
...
public class DreamPostServiceImpl implements DreamPostService
{
    ...
    @Autowired
    public DreamPostServiceImpl(final DreamPostRepository dreamPostRepository,
            ...
            for (String keyword : keywordList)
            {
                if (keyword != null && keyword.length() != 0)
                {
                    ...
                    else
                    {
                        ...
                        newKeyword.setDreamPosts(dreamPostSet);
    ...
    {
        AppUser user = appUserService.getUserByUserId(postDTO.getAppUserId());
        DreamPost existingPost = dreamPostRepository.findById(postDTO.getPostId())
                .orElseThrow(() -> new DreamPostNotFound("post not found with post id: " + postDTO.getPostId()));
    ...
    public Comment addComment(CommentDTO commentDTO)
    {
        ...
        DreamPost existingPost = dreamPostRepository.findById(commentDTO.getPostId())
                .orElseThrow(() -> new DreamPostNotFound("post not found with post id: " + commentDTO.getPostId()));
    ...
    {
        DreamPost existingPost = dreamPostRepository.findById(postId)
                .orElseThrow(() -> new DreamPostNotFound("post not found with post id: " + postId));
    ...
    {
        AppUser lAppUser = appUserService.getUserByUserId(userId);
        ...
        if (keywords != null && !keywords.isEmpty())
        {
            for (String keywordName : keywords)
            {
                ...
                {
                    Keyword keywordByName = getKeywordByName(keywordName);
                    List<DreamPost> matchedDreamPerKeyword =
                            dreamPostRepository.findByKeywordsAndAppUser(keywordByName, lAppUser);
    ...
    @Override
    public Iterable<DreamPost> getAllPost()
    {
